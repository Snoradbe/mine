...
class AddGroupHandler
{
    ...
    public function handle(User $admin, int $serverId, string $group, int $price): void
    {
        ...
        if (isset($serverGroups[$group->getName()])) {
            throw new Exception('Такая группа уже продается на этом сервере!');
        }

        $serverGroups[$group->getName()] = [
            30 => $price
        ];
        ...
        event(new AddGroupEvent($admin, $server, $group, $price));
    }
}